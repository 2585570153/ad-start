...
1. **文档处理模块** (`utils/document_loader.py`, `utils/document_processor.py`)
   - 支持PDF、Word、Excel、TXT、图片等多种格式
   - 自动文档分块，支持自定义分块大小和重叠
   - OCR图片文字识别，支持中英文
   - 文档内容清洗和格式化

2. **向量存储模块** (`database/vector_store.py`)
   - 基于ChromaDB的持久化向量存储
   - 支持知识库级别的集合管理
   - 语义相似度检索，可配置相似度阈值
   - 批量向量生成，优化性能

3. **聊天服务模块** (`services/chat_model.py`, `routers/chat.py`)
   - 流式输出支持，实时响应
...
4. **知识库管理模块** (`routers/knowledge_base.py`)
   - 知识库创建、删除、查询
   - 文档上传和批量处理
   - 向量化存储和索引
   - 文档检索和预览
